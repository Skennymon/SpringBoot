...
public class CatFactService implements Query<Integer, CatFactDTO> {
    ...
    private final String MAX_LEN = "max_length";
    ...
    public ResponseEntity<CatFactDTO> execute(Integer input) {
        //GET REQUEST
        //sets up our url using methods
        URI uri = UriComponentsBuilder.fromUriString(url).queryParam(MAX_LEN, input).build().toUri();

        //headers
        HttpHeaders headers = new HttpHeaders();
        headers.set("Accept", "application/json"); //can abstract these strings to the top of the class

        HttpEntity<String> entity = new HttpEntity<>(headers);
        //this is basically the whole http response entity and not just the body of the GET request I think
